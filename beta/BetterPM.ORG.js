...
(function(window, document, $, undefined) {
  ...
  class BetterPrivateMessages {
    ...
    deployCache(coresp) {
      ...
      this.openCache[coresp].slice(this.openCache[coresp].length > 50 ? this["openCache"][coresp].length - 50 : 0)
        .forEach((i => {
          Callbacks.pm(i, true)
        }))
    }
    ...
    saveOpen() {
      var currOpen = [];
      $("#pmbar > div[id^=pm]:not(.pm-panel-placeholder)")
        .each(function() {
          currOpen.push($(this)
            .attr("id")
            .replace(/^pm-/, ""))
